  ...
  let h0 = vacio 3 (0, 6)
   in test
        [ casilleros (agregar 0 h0)
          ...
          casilleros (agregar (-1) (agregar 1 h0))
...
testsCasilleros =
  test
    [ casilleros (vacio 3 (0, 6))
      ...
      casilleros (agregar 3 (agregar 2 (agregar 1 (vacio 4 (1, 5)))))
        ~?= [ Casillero infinitoNegativo 1 0 0.0,
              Casillero 1.0 2.0 1 33.333336,
              Casillero 2.0 3.0 1 33.333336,
              Casillero 3.0 4.0 1 33.333336,
...
testsRecr =
  test
    [
      recrExpr id (+) (ignore2 (+)) (ignore2 (-)) (ignore2 (*)) (ignore2 (/)) cuentita ~?= 4.7777777,
      recrExpr (\x -> (:[]) (show x)) (\x1 x2 -> show x1 : [show x2]) (ignore2 (++)) (ignore2 (++)) (ignore2 (++)) (ignore2 (++)) cuentita ~?= ["4.0", "3.0", "5.0", "4.0", "5.0", "9.0"]
    ] where cuentita = Div (Resta (Mult (Suma (Const 4) (Rango 3 5)) (Const 4)) (Const 5)) (Const 9)
            ignore2 f x1 x2 _ _ = f x1 x2
...
testsFold =
  test
    [
      foldExpr id (+) (+) (-) (*) (/) cuentita ~?= 4.7777777,
      foldExpr Const Rango Suma Resta Mult Div cuentita ~?= cuentita
    ] where cuentita = Div (Resta (Mult (Suma (Const 4) (Rango 3 5)) (Const 4)) (Const 5)) (Const 9)
...
testsEval =
  test
    [ fst (eval (Suma (Rango 1 5) (Const 1)) genFijo) ~?= 4.0,
      ...
      fst (eval (Suma (Rango 10 15) (Rango 5 10)) genFijo) ~?= 20.0
    ]
...
testsArmarHistograma =
  test
    [
      casilleros (fst (armarHistograma 3 3 (dameUno (1, 5)) genFijo)) ~?=
        [ Casillero infinitoNegativo 2.0 0 0.0,
          Casillero 2.0 2.6666667 0 0.0,
          Casillero 2.6666667 3.3333335 3 100.0,
...
testsEvalHistograma =
  test
    [ 
      casilleros (fst (evalHistograma 3 3 (Suma (Rango 1 5) (Const 1)) genFijo)) ~?=
        [ Casillero infinitoNegativo 3.0 0 0.0,
          Casillero 3.0 3.6666667 0 0.0,
          Casillero 3.6666667 4.3333335 3 100.0,
